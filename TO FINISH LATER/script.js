...
function createNewList(localID) {
    ...
    function populate(text) {
        for (const entries of todo[listCounter-1].items) {
            
            
            let createLi = document.createElement('li');
            let textInput = document.createTextNode(text);
            createLi.appendChild(textInput);
            document.getElementById('list' + listCounter).appendChild(createLi);
            // input.value = '';
    ...
    function createNewTask(text) {
        ...
        todo[listCounter-1].items.push(text);
